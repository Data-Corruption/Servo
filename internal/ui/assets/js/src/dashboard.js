...
function flags() {
    ...
    return {
        ...
        canBackup: el?.dataset.canBackup === '1',
...
function renderActivity(op) {
    ...
    $('activity-running')?.classList.toggle('flex', running);
...
async function refreshBackups() {
    ...
    const { canRestore, canBackup } = flags();
    for (const b of backups) {
        ...
        if (canBackup) actions.append(dl);

        if (canRestore) {
            const restore = document.createElement('button');
            restore.dataset.op = 'restore';
            restore.className = 'btn btn-warning btn-outline btn-xs op-btn';
            restore.textContent = 'Restore';
            restore.addEventListener('click', () => {
                confirmAction(
                    'Restore Backup',
                    `This will overwrite current game data with ${b.name}. The game will be stopped during restore. A failed step leaves it stopped.`,
...
async function poll() {
    ...
    try {
        const { isAdmin } = flags();
        const url = isAdmin ? `/api/status?offset=${logOffset}&id=${encodeURIComponent(operationID || '')}` : '/api/status';
    ...
    if (data.op?.id !== operationID) {
        operationID = data.op?.id;
        logOffset = 0;
        if ($('op-log')) $('op-log').textContent = '';
        if (!data.op?.running && !data.log) appendLog(data.op?.last?.tail);
        refreshBackups();
    }
    renderGame(data.game);
    const error = $('game-error');
    error.textContent = data.game?.error || '';
    error.classList.toggle('hidden', !data.game?.error);
    const checked = data.game?.checkedAt && !data.game.checkedAt.startsWith('0001-') ? new Date(data.game.checkedAt) : null;
    $('game-checked').textContent = checked ? `Status checked ${checked.toLocaleTimeString()}${Date.now() - checked.getTime() > 15000 ? ' — stale' : ''}` : 'Awaiting status probe';
    $('next-window').textContent = data.nextWindow && !data.nextWindow.startsWith('0001-') ? `Next maintenance: ${new Date(data.nextWindow).toLocaleString()} (${data.timezone} host timezone)` : 'Scheduled maintenance disabled';
...
async function startOp(op, body = {}) {
    ...
    requestPending = true; syncButtons(true);
    try {
    ...
    } finally { requestPending = false; syncButtons(wasRunning); }
}
    ...
    document.querySelectorAll('.op-btn[data-op]').forEach(btn => {
        ...
        if (op === 'stop') {
            btn.addEventListener('click', () => confirmAction(
                'Stop Game Server', 'Stop the game server? Connected players will be disconnected.',
                () => startOp('stop'), 'btn-error'));
        } else if (op === 'install') {
            btn.addEventListener('click', () => confirmAction(
                'Install Game Server',
                'Install or recreate the game runtime? The game must be stopped first. Existing game data is kept.',
                () => startOp('install')));
        } else if (op === 'update') {
            btn.addEventListener('click', () => confirmAction(
                'Update Game Server',
            ...
            btn.addEventListener('click', () => confirmAction(
                ...
                'This will stop the server, remove everything the driver installed, and delete all game data. Backup archives are kept.',
...
function syncButtons(running) {
    document.querySelectorAll('.op-btn').forEach(button => {
        ...
        const unsupported = ['restore', 'uninstall'].includes(op) && !gameState?.driverInfo?.capabilities?.includes(op);
        button.disabled = requestPending || running || !gameState?.driver || unsupported || (op === 'install' && gameState.status !== 'offline');
        button.title = unsupported ? 'This driver does not advertise support' : '';
    });
}
window.addEventListener('servo-session-expired', () => clearTimeout(pollTimer));
