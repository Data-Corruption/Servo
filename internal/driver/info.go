...
func ParseInfo(output []byte) (Info, error) {
	...
	for _, line := range strings.Split(string(output), "\n") {
		...
		switch key {
		case "CAPABILITIES":
			for _, verb := range strings.FieldsFunc(value, func(r rune) bool { return r == ',' || r == ' ' }) {
				switch verb {
				case VerbRestore, VerbUninstall, VerbNotify, VerbPlayers, VerbMetrics, VerbVersion, VerbContainerVersion:
					info.Capabilities = append(info.Capabilities, verb)
				default:
					return info, fmt.Errorf("unknown capability %q", verb)
				}
			}
		case "DRIVER_API":
...
func runCaptured(ctx context.Context, env Env, verb string, args ...string) (int, string, string, error) {
	...
	if stdout.overflow || stderr.overflow {
		err = fmt.Errorf("driver output exceeded 64 KiB")
	}
...
func List(dir string) ([]string, error) {
	if err := layout.ValidateDriverDirectory(dir); err != nil {
		if os.IsNotExist(err) {
			return nil, nil
		}
		return nil, fmt.Errorf("driver directory: %w", err)
	}
...
func (i Info) Supports(verb string) bool {
	for _, v := range i.Capabilities {
		if v == verb {
			return true
		}
	}
...
// capture bounds machine-readable output. Overflow is an error, never valid truncated data.
type capture struct {
	data     []byte
	overflow bool
}
...
func (b *capture) String() string { return string(b.data) }
