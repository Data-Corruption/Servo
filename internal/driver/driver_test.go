...
func TestCapabilitiesAndBoundedProbeOutput(t *testing.T) {
	info, err := ParseInfo([]byte("DRIVER_API=1\nNAME=fixture\nCAPABILITIES=restore,players\n"))
	if err != nil || !info.Supports(VerbRestore) || info.Supports(VerbUninstall) {
		t.Fatalf("capabilities: %+v %v", info, err)
	}
	info, err = ParseInfo([]byte("DRIVER_API=1\nNAME=fixture\n"))
	if err != nil || info.Supports(VerbRestore) {
		t.Fatal("missing metadata granted capability")
	}
	if _, err := ParseInfo([]byte("DRIVER_API=1\nNAME=fixture\nCAPABILITIES=surprise\n")); err == nil {
		t.Fatal("unknown capability accepted")
	}
	path := writeDriver(t, `head -c 70000 /dev/zero`)
	if _, err := Describe(context.Background(), testEnv(path)); err == nil || !strings.Contains(err.Error(), "64 KiB") {
		t.Fatalf("unbounded output: %v", err)
	}
}

func TestDiscoveryRejectsUnsafeDriverDirectory(t *testing.T) {
	directory := t.TempDir()
	if err := os.Chmod(directory, 0700); err != nil {
		t.Fatal(err)
	}
	linked := filepath.Join(t.TempDir(), "drivers")
	if err := os.Symlink(directory, linked); err != nil {
		t.Fatal(err)
	}
	if _, err := List(linked); err == nil {
		t.Fatal("followed symlinked driver directory")
	}
	if err := os.Chmod(directory, 0755); err != nil {
		t.Fatal(err)
	}
	if _, err := List(directory); err == nil {
		t.Fatal("accepted non-private driver directory")
	}
