...
case "$1" in describe|deps) ;; *) echo "$@" >> "$calls" ;; esac
case "$1" in
  describe) echo "DRIVER_API=1"; echo "NAME=Fixture"; echo "GAME=fixture"; echo "CAPABILITIES=restore,uninstall,notify" ;;
  deps) exit 0 ;;
...
func newFixture(t *testing.T, script string) *fixture {
	...
	db, err := database.New(filepath.Join(root, "db"), logger, build.BuildInfo{DefaultLogLevel: "warn", ServiceDefaultPort: 8829}, database.ApplyPendingMigrations)
	if err != nil {
		t.Fatal(err)
	}
	...
	runner, err := New(context.Background(), db, logger, Paths{
		DriversDir: drivers,
		DataDir:    data,
		BackupsDir: backups,
		AppVersion: "vTEST",
	...
	if env, err := runner.Env(); err != nil {
		t.Fatal(err)
	} else if err := runner.prepareEnv(env); err != nil {
		t.Fatal(err)
	}
...
func TestRestoreValidatesArchive(t *testing.T) {
	...
	if _, err := f.runner.Start(OpRestore, "nope.tar.gz"); err == nil {
...
func TestFailureStopsSequence(t *testing.T) {
	f := newFixture(t, fixtureDriver+"\n") // base fixture
...
func TestUninstallUnsupported(t *testing.T) {
	...
	if !strings.Contains(res.Detail, "exited 4") {
...
func TestActivationIsAtomicAndFailsClosed(t *testing.T) {
	f := newFixture(t, fixtureDriver)
	if err := os.WriteFile(filepath.Join(f.drivers, "other.sh"), []byte("#!/bin/sh\n"+fixtureDriver), 0755); err != nil {
		t.Fatal(err)
	}
	f.runOp(t, OpStart)
	if _, err := f.runner.Activate(context.Background(), "other.sh"); !errors.Is(err, ErrServerOnline) {
		t.Fatalf("online switch: %v", err)
	}
	f.runOp(t, OpStop)
	if _, err := f.runner.Activate(context.Background(), "other.sh"); err != nil {
		t.Fatal(err)
	}
	if cfg := mustCfg(t, f); cfg.ActiveDriver != "other.sh" {
		t.Fatal("activation not committed")
	}
	if _, err := f.runner.Activate(context.Background(), "../fixture.sh"); err == nil {
